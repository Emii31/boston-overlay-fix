...
- **Telephony (LTE/5G data, SMS/USSD, VoLTE) — V5, mixed status.** See
  `docs/telephony_investigation.md` for the VoLTE/CarrierSettings thread
  and `docs/telephony_v3_merge.md` for the SMS/USSD fix, including a
  real wrong-module mixup that was found and corrected. LTE/5G data
  stuck on EDGE: **fixed**, confirmed by direct on-device testing
  (properties in `system.prop`, not yet isolated to the specific
  load-bearing line(s)). SMS/USSD failing after boot: the real fix
  (`support_sms_over_ims_bool` / `support_ussd_over_ims_bool` set false
  via `overlay/system/etc/carrier_config_fix.xml`, plus a post-boot
  `cmd phone restart-radio`) is now correctly merged — a CRLF
  line-ending bug in the source module's `service.sh` was found and
  fixed (the restart had never actually executed in any prior test) —
  but has **not yet been confirmed working on-device** after the fix.
  The airplane-mode-toggle automation remains active as a fallback and
  only fires if the primary fix doesn't fully resolve registration.
  VoLTE/IMS registration for Grameenphone: **partially addressed, not
  yet confirmed working** — a real, schema-verified CarrierSettings
  config edit (`product/etc/CarrierSettings/s47001.pb`) is shipped,
  built on solid but not airtight evidence (a resolved 3GPP-standard
  ePDG hostname landing in Grameenphone's confirmed ASN), and has not
  yet been tested post-install. Airtel/Robi VoLTE: **still genuinely
  open** — correct MNC (02) confirmed after resolving three conflicting
  candidate values, but no working ePDG address found for it.
