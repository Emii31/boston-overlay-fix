...
mkdir -p "$MODULE_STAGING/overlay/system/etc"
...
if [ -d "$SCRIPT_DIR/overlay/system/etc" ]; then
  cp -a "$SCRIPT_DIR"/overlay/system/etc/* "$MODULE_STAGING/overlay/system/etc/" 2>/dev/null
fi
