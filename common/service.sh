...
# --- SMS/USSD radio restart (from the corrected moto-parrot-telephony-fix
# module) ---
# Source module's original service.sh had CRLF line endings, which broke
# its "until...do...done" loop on Android's shell parser (confirmed via
# `sh -n`: real syntax error, "end of file unexpected (expecting do)") -
# meaning this radio restart never actually fired in the source module
# as originally supplied. Fixed here (LF endings) and folded into this
# module's existing boot-time polling sequence rather than kept as a
# separate always-fires-regardless script: firing it here, before the
# airplane-mode poll below begins, means if the restart alone resolves
# registration, the poll correctly detects "already registered" and
# skips the toggle entirely - both mechanisms cooperate instead of
# duplicating unconditional action on every single boot.
telephony_log "Waiting for radio interface to settle before restart..."
sleep 15
cmd phone restart-radio
telephony_log "cmd phone restart-radio issued."
