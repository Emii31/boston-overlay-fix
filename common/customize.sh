...
# --- Copy system/etc file replacements into the module's mount tree ---
# Same magic-mount mechanism as the blocks above, different target
# path: /system/etc/. Two real files from the corrected
# moto-parrot-telephony-fix module ship here:
#   - carrier_config_fix.xml: real, standard AOSP CarrierConfig keys
#     (support_sms_over_ims_bool, support_ussd_over_ims_bool - both
#     false) correctly scoped to MCC 470 / MNC 01 (Grameenphone) and
#     MNC 02 (Airtel/Robi). Tells the framework to route SMS/USSD over
#     the legacy CS/GSM domain instead of depending on IMS
#     registration succeeding - see docs/telephony_v3_merge.md for the
#     full reasoning and the real logcat evidence (CS domain confirmed
#     registrationState=HOME with SMS in availableServices) that this
#     fallback path genuinely exists on this device.
#   - apns-conf.xml: real APN definitions for both carriers
#     (gpinternet / internet), matching what was independently
#     confirmed from this device's own live APN database earlier in
#     this investigation.
# Optional, same as the other blocks: absence here is not an install-time error.
mkdir -p "$MODPATH/system/etc"

unzip -o "$ZIPFILE" 'overlay/system/etc/*' -d "$TMPDIR/extracted_systemetc" >&2

if [ -d "$TMPDIR/extracted_systemetc/overlay/system/etc" ]; then
  cp -af "$TMPDIR"/extracted_systemetc/overlay/system/etc/* "$MODPATH/system/etc/" 2>/dev/null
fi

SYSTEMETC_FILE_COUNT=$(ls -1 "$MODPATH/system/etc/" 2>/dev/null | wc -l)
if [ "$SYSTEMETC_FILE_COUNT" -gt 0 ]; then
  ui_print "- Copied $SYSTEMETC_FILE_COUNT system/etc file(s) into place (carrier_config_fix.xml, apns-conf.xml)"
